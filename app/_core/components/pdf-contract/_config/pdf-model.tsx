...
export default function PDFModel({ contract }: PDFModelData) {
  ...
  const allSignatures = PdfService.getGroupSignatures([
    {
      label: "CONTRATANTE",
      name: contract.personContractorName,
      code: `CPF Nº${contract.personContractorCPF}`,
    },
    {
      label: "CONTRATADO(A)",
      name: contract.companyContractorName,
      code: `CNPJ Nº${contract.companyContractorCNPJ}`,
    },
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={{ ...styles.section, marginBottom: 30 }}>
          <Text style={styles.title}>{contract?.title ?? "CONTRATO"}</Text>
        </View>
